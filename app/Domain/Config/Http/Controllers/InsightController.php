...
class InsightController
{
    ...
    public function search(Request $request, SearchService $search): JsonResponse
    {
        ...
        if ($types !== null && ($bad = array_diff($types, array_keys(SearchService::TYPES))) !== []) {
            throw \App\Support\Http\ApiProblem::unprocessable('validation_failed', 'Unknown result type.', ['types' => ['Unknown: '.implode(', ', $bad).'. Allowed: '.implode(', ', array_keys(SearchService::TYPES)).'.']]);
        }
