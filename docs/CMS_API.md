...
* Migrations are auto-discovered. `php artisan r007:cms-seed` (idempotent; also run by `r007:demo-seed`) seeds settings defaults and demo content. Stock photos: `database/seeders/stock/` (credits in `CREDITS.md`; Unsplash and Pexels licences; every photo of people shows Black African people in Nigerian-resort scenes). To bring an ALREADY seeded database in line with a newer stock set without touching any text run `php artisan r007:cms-seed --refresh-media`: it imports new photos, repoints every reference from a replaced seeded photo (same key prefix, e.g. `hero-02-`) to its replacement, deletes the replaced media when unreferenced and fills blanks (testimonial avatars, missing album photos). It is idempotent. Demo copy is Otueke-based (near the Federal University Otueke, Bayelsa State); map coordinates 4.79 N, 6.32 E are approximate.
